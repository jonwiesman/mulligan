class EntityList
{
    ...
    addEntity(entity)
    {
        ...
        entity.z = (entity.y !== undefined) ? entity.y : 0;
...
class Aw
{
    ...
    gameLoop(curTime)
    {
        ...
        // aw.drawText({x:0, y:0, text: `${fps.toPrecision(2)} fps`, color:'white', fontSize:12, 
        //     font:"Arial", fontStyle: "normal", textAlign: "left", textBaseline: "top"});
    ...
    initEntities()
    {
        this.staticEntities = new EntityList();
        this.entities = new EntityList();
    }

    addEntity(entity, isStatic)
    {
        if(isStatic)
        {
    ...
    updateEntities(deltaTime)
    {
        this.staticEntities.updateEntities(deltaTime);
        this.entities.updateEntities(deltaTime);
    }
    ...
    renderEntities()
    {
        let i = 0;
        let j = 0;
        while(i < this.entities.entities.length && j < this.staticEntities.entities.length)
        {
            let iz = this.entities.entities[i].z;
            let jz = this.staticEntities.entities[j].z;
            if(iz < jz)
            {
                this.entities.entities[i].render();
                i++;
            }
            else
            {
                this.staticEntities.entities[j].render();
                j++;
            }
        }
        while(i < this.entities.entities.length)
        {
            this.entities.entities[i].render();
            i++;
        }
        while(j < this.staticEntities.entities.length)
        {
            this.staticEntities.entities[j].render();
            j++;
        }
    ...
    sortEntities()
    {
        this.staticEntities.sortEntities();
        this.entities.sortEntities();
    }

    clearAllEntities()
    {
        this.entities.clearAllEntities();
        this.staticEntities.clearAllEntities();
    }
    ...
    //     this.ctx.drawImage(image, -Math.floor(width * 0.5), -Math.floor(height * 0.5), width, height);
    //     this.ctx.restore();
    // }
